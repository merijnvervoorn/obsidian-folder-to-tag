...
class FolderTagPlugin extends obsidian.Plugin {
    ...
    async applyFolderTag(file, action, oldPath) {
        ...
        if (hasYAML && (action !== "create" || this.settings.overrideExisting)) {
            ...
            let yamlObj = {};
            try {
                yamlObj = jsYaml.load(yamlStr) || {};
            }
            catch (e) {
                console.error("Failed to parse YAML", e);
            }
            let tags = Array.isArray(yamlObj.tags) ? yamlObj.tags : [];
    ...
    display() {
        ...
        new obsidian.Setting(containerEl)
            ...
            drop
                ...
                .onChange(async (value) => {
                this.plugin.settings.defaultTagStyle = value;
                await this.plugin.saveSettings();
            });
        });
        // Folder depth
        ...
        new obsidian.Setting(containerEl)
            ...
            btn.onClick(async () => {
                new obsidian.Notice("Removing folder tags...");
                const files = this.plugin.app.vault.getMarkdownFiles();
                for (const file of files) {
